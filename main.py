    ...
    if USING_POSTGRES:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                name TEXT NOT NULL, email TEXT, role TEXT DEFAULT 'user'
            )
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS accounts (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                name TEXT NOT NULL, type TEXT NOT NULL, owner_user_id BIGINT
            )
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS services (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                name TEXT NOT NULL, category TEXT NOT NULL, kind TEXT NOT NULL,
                owner_user_id BIGINT, account_id BIGINT, amount NUMERIC(12,2) NOT NULL,
                frequency TEXT NOT NULL, due_date DATE NOT NULL,
                is_shared BOOLEAN NOT NULL DEFAULT FALSE, status TEXT NOT NULL DEFAULT 'pending',
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS payments (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                service_id BIGINT NOT NULL, user_id BIGINT NOT NULL, amount NUMERIC(12,2) NOT NULL,
                payment_date DATE NOT NULL, status TEXT NOT NULL DEFAULT 'paid'
            )
        """)
        conn.execute("""
...
def seed_postgres(conn):
    if execute(conn, "SELECT COUNT(*) AS count FROM services").fetchone()["count"]:
        return
